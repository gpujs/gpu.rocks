...
exports.exportsForTests = exports.convertDepGraphToDepTree = exports.inspect = void 0;
const os = require("os");
const fs = require("fs");
...
async function depsFromGraph(graphData) {
    if (!graphData || Object.keys(graphData).length === 0) {
        return {};
    }
    const pkgManager = { name: 'gradle' };
    const builder = new dep_graph_1.DepGraphBuilder(pkgManager, {
        name: 'root',
        version: '1.0.0',
    });
    ...
    const depTree = await convertDepGraphToDepTree(depGraph, 'gradle');
    return depTree.dependencies;
}
async function convertDepGraphToDepTree(depGraph, type, pruneGraph = false) {
    try {
        const depTree = (await dep_graph_1.legacy.graphToDepTree(depGraph, type, {
...
exports.convertDepGraphToDepTree = convertDepGraphToDepTree;
...
async function getAllDeps(root, targetFile, options) {
    ...
    try {
        ...
        for (const projectId in extractedJson.projects) {
            extractedJson.projects[projectId].depDict = await depsFromGraph(extractedJson.projects[projectId].depDict);
        }
