...
```js
await correlate(signal); await viaFFT();   // warm up, discard
...
Every call is `await`ed, including the five inside the loop: a kernel call
hands back a promise, so timing an unawaited one measures how long it took to
*queue* the work rather than how long the work took.
...
// The ladder: 15 passes, the stride doubling each time. Every pass is
...
async function transform(data, sign) {
  let cur = data;
  for (let ns = 1; ns < 32768; ns *= 2) cur = await fftPass(cur, ns, sign);
  return cur;
}
