...
- Each tick: accelerations first — `accel(px, py, mass, SOFT)`
- Unpack the pairs, then `stepVel(vx, vy, ax, ay, DT)`, then `stepPos` with the *new* velocities
...
Inside the loop: call `accel`, unpack its pairs into
`ax, ay` arrays (the `unpack` helper is right there), call
`stepVel`, unpack, call `stepPos`, unpack.
